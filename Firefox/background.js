// ============================================
//  BACKGROUND.JS - SoundForge v3.22.8 Firefox 153
...
//  FIREFOX 153 OPTIMIZED: полный набор иконок (active + off)
// ============================================

const browserAPI = globalThis.browser;
if (!browserAPI?.runtime) throw new Error('Mozilla Firefox extension API unavailable');

console.log('🎛️ SoundForge Background v3.22.8 Firefox 153 запущен');
...
// ============================================
//  СЕРИАЛИЗОВАННАЯ ЗАПИСЬ В STORAGE (FIREFOX 153 OPTIMIZED)
// ============================================
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      browserAPI.storage.local.get([storageKey], function(result) {
        ...
        browserAPI.storage.local.set({ [storageKey]: nextValue }, function() {
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      ...
      browserAPI.storage.local.get(keys, function(result) {
        ...
        browserAPI.storage.local.set(merged, function() {
          if (browserAPI.runtime.lastError) reject(new Error(browserAPI.runtime.lastError.message));
          else resolve(merged);
        });
      });
...
function enqueueUserPresetsMutation(mutate) {
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      browserAPI.storage.local.get(['userPresets', 'sf_userPresets'], function(result) {
        if (browserAPI.runtime.lastError) {
          reject(new Error(browserAPI.runtime.lastError.message));
          return;
        }
...
function suppressAutoConnectForTab(tabId) {
  ...
  browserAPI.tabs.get(tabId, function(tab) {
    var hadError = !!browserAPI.runtime.lastError;
...
function restorePersistedConnection(storedUrl, storedSite) {
  browserAPI.tabs.query({}, function(tabs) {
    if (browserAPI.runtime.lastError || !Array.isArray(tabs)) return;
...
function loadSavedState() {
  browserAPI.storage.local.get([
    ...
    if (browserAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка загрузки состояния:', browserAPI.runtime.lastError);
      return;
    }
...
function safeSendMessage(message) {
  try {
    browserAPI.runtime.sendMessage(message).catch(function() {});
  } catch {}
}

// ============================================
//  ОБНОВЛЕНИЕ ИКОНКИ (FIREFOX 153 OPTIMIZED)
...
function updateIcon(isActive) {
  try {
    ...
    browserAPI.action.setIcon({ path: iconPath }, function() {
      if (browserAPI.runtime.lastError) {
    ...
    if (isActive) {
      ...
      browserAPI.action.setBadgeText({ text: '⛔' });
      browserAPI.action.setBadgeBackgroundColor({ color: '#888888' });
    }
  } catch (e) {
    // Игнорируем ошибки
  }
...
// ============================================
//  ПРОВЕРКА URL (FIREFOX 153 OPTIMIZED)
// ============================================

function isSystemUrl(url) {
  if (!url) return true;
  var lowerUrl = url.toLowerCase();
  return lowerUrl.startsWith('about:') ||  
...
function findActiveTabWithAudio(callback) {
  browserAPI.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    ...
    browserAPI.tabs.query({}, function(allTabs) {
      if (browserAPI.runtime.lastError || !Array.isArray(allTabs)) {
...
function findBestTab(tabs, callback) {
  ...
  for (var j = 0; j < tabs.length; j++) {
    var tab2 = tabs[j];
    if (tab2.url && tab2.url.includes('youtube.com') && !tab2.url.startsWith('moz-extension://')) {
      callback(tab2);
      return;
    }
  }
...
// ============================================
//  ВНЕДРЕНИЕ INJECT.JS (FIREFOX 153 OPTIMIZED)
// ============================================
...
function isInjectLoaded(tabId) {
  return new Promise(function(resolve) {
    ...
    try {
      browserAPI.tabs.sendMessage(tabId, { type: 'SF_PING', data: { source: 'firefox153' } }, function(response) {
        if (browserAPI.runtime.lastError) { finish(false); return; }
...
function injectScriptDirectly(tabId, retryCount) {
  ...
  browserAPI.tabs.get(tabId, function(tab) {
    if (browserAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка получения таба', tabId, browserAPI.runtime.lastError);
      return;
    }
...
function doSendMessageToInject(tabId, type, data) {
  isInjectLoaded(tabId).then(function(loaded) {
    ...
    browserAPI.tabs.sendMessage(tabId, { type: type, data: data || {} }, function(response) {
      if (browserAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка отправки сообщения', type, browserAPI.runtime.lastError.message);
        var session = getTabSession(tabId);
        if (session) session.injected = false;
        delete state._injectedTabs[tabId];
        return;
...
function getSpectrumFromInject(tabId) {
  ...
  browserAPI.tabs.sendMessage(tabId, { type: 'SF_GET_SPECTRUM', data: {} }, function() {
    if (browserAPI.runtime.lastError) {
      var session = getTabSession(tabId);
      if (session) session.injected = false;
      delete state._injectedTabs[tabId];
...
function setTabVolumeMute(tabId, shouldMute) {
  ...
  if (shouldMute) {
    ...
    browserAPI.tabs.get(tabId, function(tab) {
      if (browserAPI.runtime.lastError || !tab) return;
      ...
      browserAPI.tabs.update(tabId, { muted: true }, function() {
        if (browserAPI.runtime.lastError) {
          console.warn('⚠️ Не удалось заглушить вкладку', tabId, browserAPI.runtime.lastError.message);
          return;
        }
  ...
  browserAPI.tabs.update(tabId, { muted: originalMuted }, function() {
    if (browserAPI.runtime.lastError) {
      console.warn('⚠️ Не удалось восстановить mute вкладки', tabId, browserAPI.runtime.lastError.message);
      return;
    }
...
function nextPreset() {
  ...
  browserAPI.storage.local.set({ selectedPreset: nextPresetName });
...
function checkRealConnectionStatus(tabId) {
  ...
  var session = getTabSession(tabId);
  browserAPI.tabs.sendMessage(tabId, { type: 'SF_PING', data: {} }, function(response) {
    if (browserAPI.runtime.lastError) {
      session.injected = false;
      session.connected = false;
      delete state._injectedTabs[tabId];
...
// ============================================
//  NAVIGATION EVENTS (FIREFOX 153 OPTIMIZED)
// ============================================
...
browserAPI.webNavigation.onCompleted.addListener(function(details) {
  if (details.frameId === 0 && details.url) {
    ...
    browserAPI.storage.local.set({ lastSite: currentSite }, function() {
      if (browserAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка сохранения lastSite:', browserAPI.runtime.lastError);
      }
    });
    ...
    loadSiteSettings(details.url).then(function(settings) {
      ...
      browserAPI.storage.local.get(['autoDisableOnSiteChange'], function(result) {
        var autoDisable = browserAPI.runtime.lastError ? true : result.autoDisableOnSiteChange !== false;
...
browserAPI.windows.onCreated.addListener(function(window) {
...
browserAPI.windows.onRemoved.addListener(function(windowId) {
...
// ============================================
//  ГОРЯЧИЕ КЛАВИШИ (4 команды) (FIREFOX 153 OPTIMIZED)
// ============================================
...
function resetAllSettings() {
  browserAPI.runtime.sendMessage({ action: 'reset', fullReset: true });
...
function getHistory() {
  return new Promise(function(resolve) {
    browserAPI.storage.local.get(['settingsHistory'], function(result) {
      if (browserAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка получения settingsHistory:', browserAPI.runtime.lastError);
        resolve([]);
        return;
      }
...
// ============================================
//  УВЕДОМЛЕНИЯ (FIREFOX 153 OPTIMIZED)
// ============================================
...
function checkNightModeAuto() {
  ...
  try {
    browserAPI.storage.local.get(['nightModeAuto'], function(result) {
      if (browserAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка получения nightModeAuto:', browserAPI.runtime.lastError);
        return;
      }
...
browserAPI.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  if (request.action === 'resize_window') {
    ...
    browserAPI.windows.getCurrent({}, function(win) {
      if (browserAPI.runtime.lastError) {
  ...
  if (request.action === 'getUserPresets') {
    browserAPI.storage.local.get(['sf_userPresets', 'userPresets'], function(result) {
      if (browserAPI.runtime.lastError) {
        sendResponse({ status: 'error', message: browserAPI.runtime.lastError.message, presets: {} });
        return;
      }
  ...
  if (request.action === 'getStatus') {
    ...
    var senderIsExtensionPage = senderUrlForStatus.startsWith('moz-extension://') ||
    ...
    if (!knownTabId) {
      ...
      browserAPI.tabs.get(knownTabId, function(tab) {
        if (browserAPI.runtime.lastError || !tab || !tab.url || !canSendMessage(tab.url) || isSystemUrl(tab.url) ||
  ...
  if (allowedActions.includes(request.action)) {
    ...
    var isFromExtension = senderUrl.startsWith('moz-extension://');
    ...
    if (explicitTargetTabId) {
      browserAPI.tabs.get(explicitTargetTabId, function(tab) {
        var invalidTarget = browserAPI.runtime.lastError || !tab || !tab.url || !canSendMessage(tab.url) || isSystemUrl(tab.url) ||
          tab.url.startsWith('moz-extension://');
        if (invalidTarget) {
...
if (browserAPI.alarms && browserAPI.alarms.create) {
  browserAPI.alarms.create('soundforge-night-mode', { periodInMinutes: 10 });
...
console.log('✅ SoundForge Background v3.22.8 Firefox 153 готов!');
