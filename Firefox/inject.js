// ============================================
//  INJECT.JS - v3.22.8 (FIREFOX 153.0esr)
...
(function() {
  ...
  const isEdge = isChrome && navigator.userAgent.toLowerCase().includes('edg/');
  
  console.log(`🎵 SoundForge v3.22.8 загружен (${isFirefox ? 'Firefox' : isEdge ? 'Edge' : 'Chrome'})`);
  ...
  function safeSendMessage(message) {
    try {
      ...
      } else if (isChrome && chrome.runtime) {
  ...
  const state = {
    ...
    _chainValid: false,
    _registeredObjects: [],
    _browser: isFirefox ? 'firefox' : 'chrome',
    _isFirefox: isFirefox,
    _isSafari: false,
    _isChromium: !isFirefox,
    _hardMute: false,
  ...
  // ============================================
  //  ЗАГРУЗКА НАСТРОЕК (localStorage)
  // ============================================
  ...
  // ============================================
  //  ПРОВЕРКА И ВОССТАНОВЛЕНИЕ ЦЕПОЧКИ
  // ============================================
