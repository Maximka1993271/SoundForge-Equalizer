//  HISTORY.JS - SoundForge v3.22.8 Chrome 152
//  Google Chrome 152.0.7977.65 | Windows 11 25H2
//  История изменений
//  Хранение до 1000 записей
//  CHROME 152 OPTIMIZED: обработка ошибок storage
// ============================================
...
export function getHistory() {
  return new Promise(function(resolve) {
    chromeAPI.storage.local.get([CONFIG.storageKey], function(result) {
      if (chromeAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка получения истории:', chromeAPI.runtime.lastError);
        resolve([]);
        return;
      }
...
function saveHistory(history) {
  return new Promise(function(resolve) {
  chromeAPI.storage.local.set({ [CONFIG.storageKey]: history }, function() {
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка сохранения истории:', chromeAPI.runtime.lastError);
    }
    resolve();
  });
  });
...
export function clearHistory() {
  chromeAPI.storage.local.set({ [CONFIG.storageKey]: [] }, function() {
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка очистки истории:', chromeAPI.runtime.lastError);
    } else {
...
export function initHistoryTracking() {
  ...
  chromeAPI.storage.onChanged.addListener(function(changes, namespace) {
