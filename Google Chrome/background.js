// ============================================
//  BACKGROUND.JS - SoundForge v3.22.8 Chrome 152
...
//  CHROME 152 OPTIMIZED: без chrome.* прямых вызовов
//  CHROME 152 OPTIMIZED: полный набор иконок (active + off)
// ============================================

const chromeAPI = globalThis.chrome;
if (!chromeAPI?.runtime) throw new Error('Google Chrome extension API unavailable');

console.log('🎛️ SoundForge Background v3.22.8 Chrome 152 запущен');
...
// ============================================
//  СЕРИАЛИЗОВАННАЯ ЗАПИСЬ В STORAGE (CHROME 152 OPTIMIZED)
// ============================================
...
function enqueueStorageMutation(storageKey, mutate) {
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      chromeAPI.storage.local.get([storageKey], function(result) {
        ...
        chromeAPI.storage.local.set({ [storageKey]: nextValue }, function() {
...
function enqueueStoragePatch(queueKey, patch) {
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      ...
      chromeAPI.storage.local.get(keys, function(result) {
        if (chromeAPI.runtime.lastError) {
          reject(new Error(chromeAPI.runtime.lastError.message));
          return;
        }
        var merged = Object.assign({}, result || {}, patch || {});
        chromeAPI.storage.local.set(merged, function() {
          if (chromeAPI.runtime.lastError) reject(new Error(chromeAPI.runtime.lastError.message));
          else resolve(merged);
        });
      });
...
function enqueueUserPresetsMutation(mutate) {
  ...
  var next = previous.then(function() {
    return new Promise(function(resolve, reject) {
      chromeAPI.storage.local.get(['userPresets', 'sf_userPresets'], function(result) {
        if (chromeAPI.runtime.lastError) {
          reject(new Error(chromeAPI.runtime.lastError.message));
          return;
        }
...
function suppressAutoConnectForTab(tabId) {
  ...
  chromeAPI.tabs.get(tabId, function(tab) {
    var hadError = !!chromeAPI.runtime.lastError;
...
function loadSavedState() {
  chromeAPI.storage.local.get([
    ...
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка загрузки состояния:', chromeAPI.runtime.lastError);
      return;
    }
...
function safeSendMessage(message) {
  try {
    chromeAPI.runtime.sendMessage(message).catch(function() {});
  } catch {}
}

// ============================================
//  ОБНОВЛЕНИЕ ИКОНКИ (CHROME 152 OPTIMIZED)
...
function updateIcon(isActive) {
  try {
    ...
    chromeAPI.action.setIcon({ path: iconPath }, function() {
      if (chromeAPI.runtime.lastError) {
    ...
    if (isActive) {
      ...
      chromeAPI.action.setBadgeText({ text: '⛔' });
      chromeAPI.action.setBadgeBackgroundColor({ color: '#888888' });
    }
  } catch (e) {
    // Игнорируем ошибки
  }
...
// ============================================
//  ПРОВЕРКА URL (CHROME 152 OPTIMIZED)
// ============================================
...
function findActiveTabWithAudio(callback) {
  chromeAPI.tabs.query({ active: true, currentWindow: true }, function(tabs) {
    ...
    chromeAPI.tabs.query({}, function(allTabs) {
      if (chromeAPI.runtime.lastError || !Array.isArray(allTabs)) {
...
// ============================================
//  ВНЕДРЕНИЕ INJECT.JS (CHROME 152 OPTIMIZED)
// ============================================
...
function isInjectLoaded(tabId) {
  return new Promise(function(resolve) {
    ...
    try {
      chromeAPI.tabs.sendMessage(tabId, { type: 'SF_PING', data: { source: 'chrome152' } }, function(response) {
        if (chromeAPI.runtime.lastError) { finish(false); return; }
...
function injectScriptDirectly(tabId, retryCount) {
  ...
  chromeAPI.tabs.get(tabId, function(tab) {
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка получения таба', tabId, chromeAPI.runtime.lastError);
      return;
    }
...
function doSendMessageToInject(tabId, type, data) {
  isInjectLoaded(tabId).then(function(loaded) {
    ...
    chromeAPI.tabs.sendMessage(tabId, { type: type, data: data || {} }, function(response) {
      if (chromeAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка отправки сообщения', type, chromeAPI.runtime.lastError.message);
        var session = getTabSession(tabId);
        if (session) session.injected = false;
        delete state._injectedTabs[tabId];
        return;
...
function getSpectrumFromInject(tabId) {
  ...
  chromeAPI.tabs.sendMessage(tabId, { type: 'SF_GET_SPECTRUM', data: {} }, function() {
    if (chromeAPI.runtime.lastError) {
      var session = getTabSession(tabId);
      if (session) session.injected = false;
      delete state._injectedTabs[tabId];
...
function setTabVolumeMute(tabId, shouldMute) {
  ...
  if (shouldMute) {
    ...
    chromeAPI.tabs.get(tabId, function(tab) {
      ...
      chromeAPI.tabs.update(tabId, { muted: true }, function() {
        if (chromeAPI.runtime.lastError) {
          console.warn('⚠️ Не удалось заглушить вкладку', tabId, chromeAPI.runtime.lastError.message);
          return;
        }
  ...
  chromeAPI.tabs.update(tabId, { muted: originalMuted }, function() {
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Не удалось восстановить mute вкладки', tabId, chromeAPI.runtime.lastError.message);
      return;
    }
...
function nextPreset() {
  ...
  chromeAPI.storage.local.set({ selectedPreset: nextPresetName });
...
function checkRealConnectionStatus(tabId) {
  ...
  var session = getTabSession(tabId);
  chromeAPI.tabs.sendMessage(tabId, { type: 'SF_PING', data: {} }, function(response) {
    if (chromeAPI.runtime.lastError) {
      session.injected = false;
      session.connected = false;
      delete state._injectedTabs[tabId];
...
// ============================================
//  NAVIGATION EVENTS (CHROME 152 OPTIMIZED)
// ============================================
...
chromeAPI.webNavigation.onCompleted.addListener(function(details) {
  if (details.frameId === 0 && details.url) {
    ...
    chromeAPI.storage.local.set({ lastSite: currentSite }, function() {
      if (chromeAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка сохранения lastSite:', chromeAPI.runtime.lastError);
      }
    });
    ...
    loadSiteSettings(details.url).then(function(settings) {
      ...
      chromeAPI.storage.local.get(['autoDisableOnSiteChange'], function(result) {
        var autoDisable = chromeAPI.runtime.lastError ? true : result.autoDisableOnSiteChange !== false;
...
chromeAPI.webNavigation.onHistoryStateUpdated.addListener(function(details) {
  if (details.frameId === 0 && details.url) {
...
chromeAPI.windows.onCreated.addListener(function(window) {
...
chromeAPI.windows.onRemoved.addListener(function(windowId) {
...
// ============================================
//  ГОРЯЧИЕ КЛАВИШИ (4 команды) (CHROME 152 OPTIMIZED)
// ============================================
...
function resetAllSettings() {
  chromeAPI.runtime.sendMessage({ action: 'reset', fullReset: true });
...
function getHistory() {
  return new Promise(function(resolve) {
    chromeAPI.storage.local.get(['settingsHistory'], function(result) {
      if (chromeAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка получения settingsHistory:', chromeAPI.runtime.lastError);
        resolve([]);
        return;
      }
...
// ============================================
//  УВЕДОМЛЕНИЯ (CHROME 152 OPTIMIZED)
// ============================================
...
function checkNightModeAuto() {
  ...
  try {
    chromeAPI.storage.local.get(['nightModeAuto'], function(result) {
      if (chromeAPI.runtime.lastError) {
        console.warn('⚠️ Ошибка получения nightModeAuto:', chromeAPI.runtime.lastError);
        return;
      }
...
// ============================================
//  ОБРАБОТЧИК СООБЩЕНИЙ (CHROME 152 OPTIMIZED)
// ============================================
...
chromeAPI.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  // ============================================
  //  FULLSCREEN: RESIZE WINDOW (Chrome/Chromium)
  // ============================================
  if (request.action === 'resize_window') {
    console.log('🪟 Resizing window:', request.width, 'x', request.height);
    chromeAPI.windows.getCurrent({}, function(win) {
      if (chromeAPI.runtime.lastError) {
        sendResponse({ status: 'error', message: chromeAPI.runtime.lastError.message });
        return;
      }
      chromeAPI.windows.update(win.id, {
        width: request.width,
  ...
  if (request.action === 'getUserPresets') {
    chromeAPI.storage.local.get(['sf_userPresets', 'userPresets'], function(result) {
      if (chromeAPI.runtime.lastError) {
        sendResponse({ status: 'error', message: chromeAPI.runtime.lastError.message, presets: {} });
        return;
      }
  ...
  if (request.action === 'exportSettings') {
    chromeAPI.storage.local.get(null, function(data) {
      if (chromeAPI.runtime.lastError) {
        sendResponse({ status: 'error', message: chromeAPI.runtime.lastError.message });
        return;
      }
  ...
  if (request.action === 'getStatus') {
    ...
    if (!knownTabId) {
      ...
      chromeAPI.tabs.get(knownTabId, function(tab) {
        if (chromeAPI.runtime.lastError || !tab || !tab.url || !canSendMessage(tab.url) || isSystemUrl(tab.url) ||
  ...
  if (allowedActions.includes(request.action)) {
    ...
    if (explicitTargetTabId) {
      chromeAPI.tabs.get(explicitTargetTabId, function(tab) {
        var invalidTarget = chromeAPI.runtime.lastError || !tab || !tab.url || !canSendMessage(tab.url) || isSystemUrl(tab.url) ||
          tab.url.startsWith('chrome-extension://') || tab.url.startsWith('moz-extension://');
        if (invalidTarget) {
    ...
    chromeAPI.tabs.query({ active: true, currentWindow: true }, function(tabs) {
      if (chromeAPI.runtime.lastError || !tabs || tabs.length === 0) {
        sendResponse({ status: 'no_tab' });
        return;
      }
...
if (chromeAPI.alarms && chromeAPI.alarms.create) {
  chromeAPI.alarms.create('soundforge-night-mode', { periodInMinutes: 10 });
...
console.log('✅ SoundForge Background v3.22.8 Chrome 152 готов!');
