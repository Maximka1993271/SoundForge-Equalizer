...
console.log('🎛️ SoundForge Popup v3.22.8 Chrome 152 - Единое хранилище (с эффектами)');
...
function restoreConnectionState() {
  ...
  chromeAPI.runtime.sendMessage({ action: 'getStatus' }, function(response) {
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Ошибка получения статуса:', chromeAPI.runtime.lastError.message);
...
function applyPresetInPopup(name) {
  ...
  chromeAPI.runtime.sendMessage({
    ...
    if (chromeAPI.runtime.lastError) {
      console.warn('⚠️ Не удалось применить пресет:', chromeAPI.runtime.lastError.message);
      return;
    }
...
// ============================================
//  ПОДКЛЮЧЕНИЕ/ОТКЛЮЧЕНИЕ (CHROME 152 OPTIMIZED)
// ============================================
...
function handleConnectDisconnect() {
  ...
  if (disconnecting) {
    chromeAPI.runtime.sendMessage({ action: 'disconnect', targetTabId: popupTargetTabId }, function(response) {
      finishPopupConnectionLoading();
      if (response && response.tabId != null) popupTargetTabId = Number(response.tabId);
      if (chromeAPI.runtime.lastError || !response || response.status !== 'disconnected') {
  ...
  state.isConnected = false;
  setStatus('connecting', t('status_connecting'));
  chromeAPI.runtime.sendMessage({ action: 'connect', targetTabId: popupTargetTabId }, function(response) {
    if (response && response.tabId != null) popupTargetTabId = Number(response.tabId);
    ...
    if (response.status === 'connecting') {
      ...
      popupConnectWatchdog = setTimeout(function() {
        chromeAPI.runtime.sendMessage({ action: 'getStatus', targetTabId: popupTargetTabId }, function(statusResponse) {
          finishPopupConnectionLoading();
          if (chromeAPI.runtime.lastError || !statusResponse) {
            state.isConnected = false;
            setStatus('error', formatConnectionError());
            return;
          }
...
function setupNewFeatureButtons() {
  ...
  if (nightBtn) {
    nightBtn.addEventListener('click', function() {
      chromeAPI.runtime.sendMessage({ action: 'toggleNightMode' }, function(response) {
  ...
  if (powerBtn) {
    powerBtn.addEventListener('click', function() {
      chromeAPI.runtime.sendMessage({ action: 'togglePowerSave' }, function(response) {
        if (response && response.enabled !== undefined) {
  ...
  if (windowBtn) {
    windowBtn.addEventListener('click', function() {
      chromeAPI.runtime.sendMessage({ action: 'open_window' });
    });
  }
  ...
  if (historyBtn) {
    historyBtn.addEventListener('click', function() {
      chromeAPI.runtime.sendMessage({ action: 'getHistory' }, function(response) {
...
chromeAPI.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  if (request.action === 'statusUpdate') {
    ...
    if (request.status === 'connected') {
      ...
      setStatus('connected', t('status_connected'));
      state.isConnected = true;
      chromeAPI.storage.local.set({ soundforgeConnected: true });
    } else if (request.status === 'disconnected') {
      finishPopupConnectionLoading();
      setStatus('disconnected', t('status_disconnected'));
      state.isConnected = false;
      chromeAPI.storage.local.set({ soundforgeConnected: false });
...
async function initPopup() {
  ...
  if (dom.eqSliders) {
    dom.eqSliders.forEach(function(slider) {
      ...
      slider.addEventListener('input', function() {
        ...
        chromeAPI.runtime.sendMessage({ action: 'updateEQ', gains: gains, instant: true });
        saveAllSettings();
        state.currentPreset = 'custom';
        updatePresetInfo('custom');
      ...
      slider.addEventListener('change', function() {
        var gains = getSliderGains();
        chromeAPI.runtime.sendMessage({ action: 'updateEQ', gains: gains });
        saveAllSettings();
        updateEQGraph();
      });
    });
  ...
  if (dom.volumeSlider && dom.volumeDisplay) {
    ...
    dom.volumeSlider.addEventListener('change', function() {
      var val = parseInt(this.value);
      var volumeValue = val / 100;
      chromeAPI.runtime.sendMessage({ action: 'setVolume', value: volumeValue });
      saveAllSettings();
  ...
  if (dom.bassSlider && dom.bassDisplay) {
    dom.bassSlider.addEventListener('input', function() {
      ...
      chromeAPI.runtime.sendMessage({ action: 'setBass', value: val, instant: true });
      saveAllSettings();
    });
    dom.bassSlider.addEventListener('change', function() {
      var val = parseFloat(this.value);
      chromeAPI.runtime.sendMessage({ action: 'setBass', value: val });
      saveAllSettings();
    });
  }
  ...
  setTimeout(function() {
    chromeAPI.runtime.sendMessage({ action: 'getSpectrum' });
    console.log('📊 Запрошен спектр');
  }, 250);

  popupStatusInterval = setInterval(function() {
    chromeAPI.runtime.sendMessage({ action: 'getStatus', targetTabId: popupTargetTabId }, function(response) {
  ...
  chromeAPI.runtime.sendMessage({ action: 'getUserPresets' });
  ...
  console.log('✅ SoundForge Popup v3.22.8 Chrome 152 готов!');
...
function cleanupPopupRuntime() {
  ...
  try {
    chromeAPI.runtime.sendMessage({ action: 'unregisterSpectrumListener', clientId: spectrumClientId }, function() {
