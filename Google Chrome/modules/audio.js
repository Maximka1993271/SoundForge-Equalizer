// ============================================
//  AUDIO.JS - SoundForge v3.22.8 Chrome 152
...
export async function applyPresetWithAnimation(name, isUserPreset = false) {
    ...
    try {
        ...
        chromeAPI.runtime.sendMessage({ action: 'setVolume', value: targetVolume / 100, instant: true });
        chromeAPI.runtime.sendMessage({ action: 'setBass', value: targetBass, instant: true });
    } catch (e) {}
    saveAllSettings();
...
export function toggleConnection() {
    ...
    if (state.currentStatus === 'connected') {
        ...
        chromeAPI.runtime.sendMessage({ action: 'connect' }, () => {
            showLoading(false);
            if (chromeAPI.runtime.lastError) {
                setStatus('disconnected', '⚠️ Ошибка: ' + chromeAPI.runtime.lastError.message);
                return;
            }
            
            setTimeout(() => {
                chromeAPI.runtime.sendMessage({ action: 'getStatus' }, (resp) => {
                    if (resp && resp.status === 'connected') {
                        setStatus('connected', t('status_connected'));
                        applySavedSettings(false);
                    } else {
                        setTimeout(() => {
                            chromeAPI.runtime.sendMessage({ action: 'getStatus' }, (resp2) => {
...
export async function handleReset(onVolumeReset = null) {
    ...
    try {
        ...
        try {
            chromeAPI.runtime.sendMessage({ action: 'updateEQ', gains: gains, instant: true });
            chromeAPI.runtime.sendMessage({ action: 'setVolume', value: 1.0, instant: true });
            chromeAPI.runtime.sendMessage({ action: 'setBass', value: 0, instant: true });
        } catch (e) {}
...
export function applySavedSettings(applyPresetToo = true) {
    chromeAPI.storage.local.get([
        ...
        if (savedVolume !== undefined && dom.volumeSlider && dom.volumeDisplay) {
            ...
            chromeAPI.runtime.sendMessage({ action: 'setVolume', value: vol / 100 });
        ...
        if (savedBass !== undefined && dom.bassSlider && dom.bassDisplay) {
            ...
            chromeAPI.runtime.sendMessage({ action: 'setBass', value: bass });
