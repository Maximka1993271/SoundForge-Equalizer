// ============================================
//  STORAGE.JS - SoundForge v3.22.8 Chrome 152
//  Google Chrome 152.0.7977.65 | Windows 11 25H2
//  Работа с хранилищем
//  CHROME 152 OPTIMIZED: обработка ошибок в exportSettings
//  CHROME 152 OPTIMIZED: единое хранилище через storage-sync
// ============================================
...
function sendPresetMutation(action, payload) {
  try {
    const maybePromise = chromeAPI.runtime.sendMessage({ action, ...payload });
...
export function saveAllSettings() {
  ...
  try {
    chromeAPI.runtime.sendMessage({ action: 'settingsSnapshot', settings: patch }, (response) => {
      if (chromeAPI.runtime.lastError || !response || response.status !== 'ok') {
        fallback();
      }
    });
  } catch (error) {
...
export function loadSettings(callback) {
  ...
  chromeAPI.storage.local.get(keys, (result) => {
    if (chromeAPI.runtime.lastError) {
      console.warn('[Storage] Ошибка загрузки настроек:', chromeAPI.runtime.lastError);
      callback?.({});
      return;
    }
    const normalized = {
...
export function importSettings(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      chromeAPI.runtime.sendMessage({ action: 'settingsSnapshot', settings: safeSettings }, (response) => {
        if (chromeAPI.runtime.lastError) {
          reject(new Error(chromeAPI.runtime.lastError.message));
          return;
        }
