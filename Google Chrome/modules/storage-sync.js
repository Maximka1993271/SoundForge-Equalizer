// ============================================
//  STORAGE-SYNC.JS - SoundForge v3.22.8 Chrome 152
//  Google Chrome 152.0.7977.65 | Windows 11 25H2
//  Единая система хранения
//  Версия: 1.0.1
//  Обеспечивает синхронизацию между localStorage и chromeAPI.storage
//  CHROME 152 OPTIMIZED: обработка ошибок во всех методах
//  CHROME 152 OPTIMIZED: fallback при недоступности localStorage
// ============================================
...
class StorageManager {
  constructor() {
    ...
    try {
      if (typeof chromeAPI !== 'undefined' && chromeAPI.storage?.onChanged) {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!data || Object.keys(data).length === 0) {
        ...
        await this._saveToChrome(canonicalDefaults);
        await this._updateCache(canonicalDefaults);
      } else {
        await this._updateCache(data);
        console.log('[Storage] Настройки загружены из chromeAPI.storage');
      }
  ...
  /**
   * Обновить локальный кэш без прямой записи в chromeAPI.storage.
      ...
      if (typeof chromeAPI !== 'undefined' && chromeAPI.storage) {
        chromeAPI.storage.local.get(null, (result) => {
          if (chromeAPI.runtime.lastError) {
            console.warn('[Storage] Ошибка загрузки из chromeAPI.storage:', chromeAPI.runtime.lastError);
            resolve(null);
          } else {
            resolve(result);
          }
      ...
      if (typeof chromeAPI !== 'undefined' && chromeAPI.storage) {
        chromeAPI.storage.local.set(data, () => {
          if (chromeAPI.runtime.lastError) {
            ...
            console.warn('[Storage] Ошибка сохранения в chromeAPI.storage:', error);
            reject(error);
            return;
          }
          resolve();
        ...
        reject(new Error('chromeAPI.storage.local недоступен'));
      }
    });
  }
    ...
    return new Promise((resolve) => {
      if (typeof chromeAPI !== 'undefined' && chromeAPI.storage) {
        chromeAPI.storage.local.clear(() => {
          if (chromeAPI.runtime.lastError) {
            console.warn('[Storage] Ошибка очистки chromeAPI.storage:', chromeAPI.runtime.lastError);
          }
          resolve();
        });
      } else {
  ...
  async _syncFromLocalStorage() {
    if (!this._hasLocalStorage) return;
    try {
      let current = await this._loadFromChrome();
