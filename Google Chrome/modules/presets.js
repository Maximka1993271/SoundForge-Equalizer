// ============================================
//  PRESETS.JS - SoundForge v3.22.8 Chrome 152
//  Google Chrome 152.0.7977.65 | Windows 11 25H2
//  Работа с пресетами
//  CHROME 152 OPTIMIZED: выбор пресетов из списка
//  CHROME 152 OPTIMIZED: определение пользовательских пресетов
//  CHROME 152 OPTIMIZED: обработка ошибок и очистка A/B состояния
// ============================================
...
function applyABSnapshot(snapshot, side) {
  ...
  const gains = getSliderGains();
  chromeAPI.runtime.sendMessage({ action: 'updateEQ', gains, instant: true, source: `ab-${side}` });
  chromeAPI.runtime.sendMessage({ action: 'setVolume', value: volumePercent / 100, instant: true, source: `ab-${side}` });
  chromeAPI.runtime.sendMessage({ action: 'setBass', value: bass, instant: true, source: `ab-${side}` });
