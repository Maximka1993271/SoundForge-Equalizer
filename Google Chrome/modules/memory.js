// ============================================
//  MEMORY.JS - SoundForge v3.22.8 Chrome 152
//  Google Chrome 152.0.7977.65 | Windows 11 25H2
//  Управление памятью
//  Версия: 1.0.0
//  CHROME 152 OPTIMIZED: очистка мониторинга при уничтожении
...
export class MemoryManager {
  constructor() {
    ...
    this._chromeMemoryAPI = null;
    
    // Проверка поддержки memory API в Chrome
    this._checkMemoryAPI();
  }

  /**
   * Проверка поддержки performance.memory в Chrome
   */
  _checkMemoryAPI() {
    try {
      if (typeof performance !== 'undefined' && performance.memory) {
        this._chromeMemoryAPI = performance.memory;
      }
    } catch (e) {
      // Игнорируем
    }
  ...
  /**
   * Безопасный запрос сборки мусора (Chrome/Chromium)
   */
  _requestGC() {
    try {
      // Chrome/Chromium: window.gc() может быть доступен с флагом
  ...
  getStats() {
    ...
    if (this._chromeMemoryAPI) {
      stats.memoryInfo = {
        totalJSHeapSize: this._chromeMemoryAPI.totalJSHeapSize,
        usedJSHeapSize: this._chromeMemoryAPI.usedJSHeapSize,
        jsHeapSizeLimit: this._chromeMemoryAPI.jsHeapSizeLimit
      };
