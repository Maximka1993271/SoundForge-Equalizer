// ============================================
//  INJECT.JS - SoundForge v3.22.8 Chrome 152
...
//  CHROME 152 OPTIMIZED: файловая инъекция через SF_PING
// ============================================
...
(function() {
  ...
  console.log('🎵 SoundForge v3.22.8 Chrome 152 загружен');
  ...
  function requestRuntimeMessage(message) {
    return new Promise((resolve) => {
      try {
        if (typeof chromeAPI === 'undefined' || !chromeAPI.runtime?.sendMessage) {
          resolve(null);
          return;
        }
  ...
  // ============================================
  //  AUDIOCONTEXT ДЛЯ CHROME 152 AUTOPLAY POLICY
  // ============================================
  ...
  function ensureAudioContextRunning() {
    ...
    return context.resume().then(() => context.state === 'running').catch(() => {
      if (!state._chromeResumeArmed) {
        state._chromeResumeArmed = true;
        const resumeFromGesture = () => {
          state._chromeResumeArmed = false;
  ...
  function handleConnect(isAutomatic) {
    ...
    log('🔗 РУЧНОЕ ПОДКЛЮЧЕНИЕ (Google Chrome 152)', 'info');
  ...
  // ============================================
  //  RUNTIME MESSAGE LISTENER (CHROME 152 OPTIMIZED)
  // ============================================

  if (typeof chromeAPI !== 'undefined' && chromeAPI.runtime?.onMessage) {
  ...
  function init() {
    ...
    log('✅ SoundForge v3.22.8 Chrome 152 инициализирован', 'info');
    observeChanges();
  ...
  log('✅✅✅ SoundForge v3.22.8 Chrome 152 загружен ✅✅✅', 'info');
